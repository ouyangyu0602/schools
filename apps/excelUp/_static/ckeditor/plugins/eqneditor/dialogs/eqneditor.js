				...
				elements : [
	  			{
					...
					{
							type: 'html',
							html: '<label for="CClatex'+window.CCounter+'">Edit(LaTeX):</label>',
					},
					{
							type: 'html',
					...
					{
						 	type: 'html',
							html: '<label for="CCequation'+window.CCounter+'">Show:</label>'		
					},
					{
							type :'html',
...
//			editor.document.write('\\('+EqEditor.getTextArea().getLaTeX()+'\\)');
			Example.add_history(EqEditor.getTextArea().getLaTeX());
			/*var eqn = editor.document.createElement( '<span>' );
			eqn.innerText(EqEditor.getTextArea().getLaTeX());
			editor.insertElement(eqn);
			Example.add_history(EqEditor.getTextArea().getLaTeX());
			//editor.write(eqn);*/
		}
