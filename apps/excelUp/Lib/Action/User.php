...
if(isset($_REQUEST['staff_id']) && $_REQUEST['staff_id']!='new')
{
	...
	//	echo '<IMG SRC=assets/pixel_trans.gif height=2>';
            ...
            if($count_staff_RET[1]['NUM']>1){
                DrawHeaderHome( '所选用户: '.$RET[1]['FIRST_NAME'].'&nbsp;'.$RET[1]['LAST_NAME'].' (<A HREF=Side.php?staff_id=new&modcat='.$_REQUEST['modcat'].'><font color=red>取消选择</font></A>) | <A HREF=Modules.php?modname='.$_REQUEST['modname'].'&search_modfunc=list&next_modname=Users/User.php&ajax=true&bottom_back=true&return_session=true target=body>返回用户列表</A>');
            }else{
                DrawHeaderHome( '所选用户: '.$RET[1]['FIRST_NAME'].'&nbsp;'.$RET[1]['LAST_NAME'].' (<A HREF=Side.php?staff_id=new&modcat='.$_REQUEST['modcat'].'><font color=red>取消选择</font></A>)');
            }
}
#############################################
...
//                    }
                        ...
                        if($number!=0)
                        {
                            echo '<font color = red><b>无效密码</b></font>';
                        }
		       if($number==0)
		        {
			...
			if($_REQUEST['staff']['USERNAME'] && $_REQUEST['staff']['USERNAME']!=$profile_RET[1]['USERNAME'])
			{
				...
				if(count($existing_staff))
					BackPrompt('在 '.$existing_staff[1]['SYEAR'].' 学年已经存在该用户名. 选择一个不同的用户名，然后再试一次.');
			}
                                                      $go=false;
			...
			foreach($_REQUEST['staff'] as $column_name=>$value)
				...
				echo "<font color=red><b>不能保存数据, 因为 ".$custom_TITLE.' 是必需的.</b></font><br/>';
				$error=true;
				}else{
...
//                               }
                        ...
                        {
                            $err= "<center><font color=red><b>无效的数据不能被保存.</b><font></center>";
                        }
...
//                    DBQuery("INSERT INTO staff_school_relationship(staff_id,school_id,syear)VALUES{$rel_value}");
		...
		else
		{
			...
			if($flag1 == 0 && $error!=true)
			{
				...
				//ADD START BY BAOYT
                //关联ts_user表，向其中添加用户信息
				$ts_user_id = DBGet(DBQuery('SELECT max(uid) as MAXUID FROM ts_user'));
				$ts_user_num = $ts_user_id[1]['MAXUID'];

				$password_rs = md5(md5($_REQUEST[staff][PASSWORD]).'11111');
				$ori_password_rs = $_REQUEST[staff][PASSWORD];
				$login_rs = $_REQUEST[staff][EMAIL];
				$uname_rs = $_REQUEST[staff][USERNAME];
				$email_rs = $_REQUEST[staff][EMAIL];
				$names_rs = $_REQUEST[staff][LAST_NAME].$_REQUEST[staff][FIRST_NAME];

				$sql_ts11 = "INSERT INTO ts_user VALUES(";
				$sql_ts11 .= "'".($ts_user_num + 1)."'";
				$sql_ts11 .= " ,'".$login_rs."'";
				$sql_ts11 .= " ,'".$password_rs."'";
				$sql_ts11 .= " ,'11111'";
				$sql_ts11 .= " ,'".$uname_rs."'";
				$sql_ts11 .= " ,'".$email_rs."'";
				$sql_ts11 .= " ,'1'";
				$sql_ts11 .= " ,'北京市 北京市 海淀区'";
				$sql_ts11 .= " ,'1'";
				$sql_ts11 .= " ,'1'";
				$sql_ts11 .= " ,'1'";
				$sql_ts11 .= " ,NULL";
                ...
                //关联ofuser表，向其中添加用户信息
				$sql_ts12 = "INSERT INTO ofuser VALUES(";
				$sql_ts12 .= " '".$uname_rs."'";
				$sql_ts12 .= " ,'".$ori_password_rs."'";
				$sql_ts12 .= " ,NULL";
				$sql_ts12 .= " ,'".$names_rs."'";
				$sql_ts12 .= " ,'".$email_rs."'";
				$sql_ts12 .= " ,'". $cur_date ."'";
				...
				//echo "<font color=red>output it</font>";
			...
			else
			{
				if($required_faild_error==true)
                                                                            ...
                                                                            echo '<font color = red><b>指定的用户名已经被占用! 请尝试使用其他用户名.</b></font>';
				$_REQUEST['staff_id'] = 'new';
			}
        ...
        if($day_valid==false)
        {
            echo "<center><font color=red><b>无效的数据不能被保存.</b><font></center>";
        }
        if($error=='end_date')
        {
            echo "<font color=red><b>开始时间不能晚于结束时间</b></font><br/>";
            unset($error);
        ...
        if($error=='start_date')
        {
            echo "<font color=red><b>开始时间不能为空</b></font><br/>";
            unset($error);
//            if($_REQUEST['staff_id']=='new')
//            {
                #header("location:/modules/Users/User.php&staff_id=new");
...
//        echo $parameter;
	...
	echo '<CENTER>'.SubmitButton('保存','','class=btn_medium onclick="return formcheck_user_user('.$_SESSION[staff_school_chkbox_id].');"').'</CENTER>';
    // echo '测试一下';

    // echo '------'.$_REQUEST['staff']['PASSWORD'].'<br/>';
    // echo '------'.$_REQUEST['staff']['EMAIL'];
